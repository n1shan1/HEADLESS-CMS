  ...
  db.query(
    ...
    (err, entityResult) => {
      if (err) {
        ...
        res
          .status(500)
          .json({ error: "An error occurred during entity creation" });
        return;
      }
      ...
      for (const attribute of attributes) {
        db.query(
          "INSERT INTO attributes (entity_id, name, data_type) VALUES (?, ?, ?)",
          [entityId, attribute.name, attribute.dataType],
          (err, attributeResult) => {
            if (err) {
              console.error("Error creating attribute:", err);
              res
                .status(500)
                .json({ error: "An error occurred during attribute creation" });
              return;
            }
          }
        );
      ...
      res
        .status(201)
        .json({ message: "Entity created successfully", id: entityId });
    }
  );
});
            ...
            db.query(
              ...
              (err) => {
                if (err) {
                  console.error("Error creating attribute:", err);
                  res.status(500).json({
                    error: "An error occurred during attribute creation",
                  });
                  return;
                }
              }
...
app.post("/api/entities/:entityId/attributes/:attributeId/data", (req, res) => {
  ...
  db.query(
    "INSERT INTO data (entity_id, attribute_id, value_datetime, value_text) VALUES (?, ?, ?, ?)",
    [entityId, attributeId, value_datetime, value_text],
    (err, attributeData) => {
      if (err) {
        console.error("Error entering attribute data:", err);
        res
          .status(500)
          .json({ error: "An error occurred during entering attribute data" });
        return;
      }
      const dataId = attributeData.insertId;
      res
        .status(201)
        .json({ message: "Attribute data entered successfully", id: dataId });
    }
  );
});
